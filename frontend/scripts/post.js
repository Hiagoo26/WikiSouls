...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        <article class="post">
          <div class="post-header">
            <div style="display:flex; gap:10px; align-items:center;">
              ...
              <a href="perfil.html?id=${post.autor?.id}" style="text-decoration: none; color: inherit;">
      ...
      if (likeBtn) {
        likeBtn.addEventListener("click", async (e) => {
          try {
            if (!user) return showCustomAlert("Faça login para curtir.");
            await apiFetch(`${API}/${postId}/like`, { method: "POST", body: { autorId: user.id } });
            await carregarPost();
          } catch (err) {
            console.error("Erro like post:", err);
            showCustomAlert("Erro ao curtir post: " + err.message);
          }
        });
      }
      ...
      if (deletarBtn) {
        deletarBtn.addEventListener("click", async () => {
          ...
          if (!confirmed) return;
          try {
            await apiFetch(`${API}/${postId}`, { method: "DELETE", body: { autorId: user.id } });
            window.location.href = "comunidade.html";
          } catch (err) {
            console.error("Erro deletar post:", err);
            showCustomAlert("Erro ao deletar post: " + err.message);
          }
        });
      }
      ...
      if (editarBtn) {
        editarBtn.addEventListener("click", async () => {
          ...
          try {
            await apiFetch(`${API}/${postId}`, { method: "PUT", body: { titulo: novoTitulo, conteudo: novoConteudo, autorId: user.id } });
            carregarPost();
          } catch (err) {
            console.error("Erro editar post:", err);
            showCustomAlert("Erro ao editar post: " + err.message);
          }
        });
      }
    } catch (err) {
    ...
    try {
          ...
          return `
            <div class="comentario" data-id="${c.id}">
              <div style="display:flex; gap:10px; align-items:center;">
                ...
                <a href="perfil.html?id=${c.autor?.id}" style="text-decoration: none; color: inherit;">
      ...
      listaComentarios.querySelectorAll(".like-comment").forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          ...
          if (!user) return showCustomAlert("Faça login para curtir.");
          try {
            await apiFetch(`${API}/comentario/${id}/like`, { method: "POST", body: { autorId: user.id } });
            carregarComentarios();
          } catch (err) {
            console.error("Erro curtir comentário:", err);
            showCustomAlert("Erro ao curtir comentário: " + err.message);
          }
        });
      });
      ...
      listaComentarios.querySelectorAll(".delete-comment").forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          ...
          if (!confirmed) return;
          try {
            await apiFetch(`${API}/comentario/${id}`, { method: "DELETE", body: { autorId: user.id } });
            carregarComentarios();
          } catch (err) {
            console.error("Erro deletar comentário:", err);
            showCustomAlert("Erro ao deletar comentário: " + err.message);
          }
        });
      });
      ...
      listaComentarios.querySelectorAll(".edit-comment").forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          ...
          if (novo === null) return;
          try {
            await apiFetch(`${API}/comentario/${id}`, { method: "PUT", body: { conteudo: novo, autorId: user.id } });
            carregarComentarios();
          } catch (err) {
            console.error("Erro editar comentário:", err);
            showCustomAlert("Erro ao editar comentário: " + err.message);
          }
        });
      });
    } catch (err) {
  ...
  enviarComentario.addEventListener("click", async () => {
    try {
      ...
      await apiFetch(`${API}/${postId}/comentarios`, { method: "POST", body: { conteudo: texto, autorId: user.id } });
  ...
  function showCustomConfirm(message) {
    return new Promise((resolve) => {
      ...
      okBtn.style.cssText = "padding:8px 16px; background:#dc3545; color:white; border:none; border-radius:5px; cursor:pointer;";
